...
def localization_collate(batch):
    ...
    for sample in batch:
        imgs.append(sample[0])
        object_imgs.append(sample[1])
        targets.append(torch.FloatTensor(sample[2]))
    return torch.stack(imgs, 0), torch.stack(object_imgs, 0), torch.stack(targets)
...
class VOCLocalization(data.Dataset):
    """VOC Detection Dataset Object
    ...
    def __init__(self, root,
                 image_sets=[('2007', 'trainval'), ('2012', 'trainval')],
                 transform=None, target_transform=VOCAnnotationTransform(),
                 dataset_name='VOC0712', object_size=64):
        self.root = root
        self.image_set = image_sets
        self.transform = transform
        self.target_transform = target_transform
