...
train_loader = torch.utils.data.DataLoader(
    COCOLocalization(root='/home1/mxj/workspace/dataset/COCO/2017',
...
def train(model, loader, epoch, optimizer):
    ...
    for batch_idx, (img, object_img, target) in enumerate(loader):
        ...
        if batch_idx % args.log_interval == 0:
            ...
            print(label_id)
            if 0 <= x < min_dim and 0 <= y < min_dim:
                log_img = cv2.circle(log_img, (x, y), 10, (255,0,0), 5)
                cv2.putText(log_img, COCO_CLASSES[label_id], (x, y), cv2.FONT_HERSHEY_SIMPLEX, 0.3, 
                        (255,0,0))
            log_img = cv2.circle(log_img, (gt_x, gt_y), 10, (0,255,0), 5)
            cv2.putText(log_img, COCO_CLASSES[label_id], (gt_x, gt_y), cv2.FONT_HERSHEY_SIMPLEX, 0.3, 
                    (0,255,0))
